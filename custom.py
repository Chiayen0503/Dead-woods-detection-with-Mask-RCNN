...
class DatasetConfig(Config):
    ...
    NUM_CLASSES = 1 + 1 # Background + labels 
...
def train(model):
    ...
    model.train(dataset_train, dataset_val,
                ...
                epochs=30,   #10
                layers='head', augmentation = augmentation)
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        train(model)
        history = model.keras_model.history.history  #defalut = no this line
        epochs = range(1,len(next(iter(history.values())))+1)
        df = pd.DataFrame(history, index=epochs)
        df.to_csv("score.csv", header = True)
